...
timeout 120 docker compose -p "$PAPERCLIP_STAGING_PROJECT" -f "$repo_root/docker/staging/compose.yml" down --remove-orphans
if [[ "${PAPERCLIP_STAGING_DELETE_DATA:-false}" == "true" ]]; then
  timeout 60 docker volume rm "${PAPERCLIP_STAGING_PROJECT}_db" "${PAPERCLIP_STAGING_PROJECT}_runtime"
fi
