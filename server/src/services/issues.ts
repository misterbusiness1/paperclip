...
async function blockedInboxIssueConditions(
  ...
  if (filters?.parentId) {
    // A non-UUID parentId (e.g. a human identifier like "OXFA-123") would fail
    // the uuid cast with a 500; no issue can have such a parent, so match none.
    conditions.push(
      isUuidLike(filters.parentId) ? eq(issues.parentId, filters.parentId) : sql`false`,
    );
  }
