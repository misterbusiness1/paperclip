...
const RESOLVED_DEPENDENCY_WAKE_BACKSTOP_CANDIDATE_LIMIT = 2000;
...
const PROVIDER_QUOTA_RESET_MONTHS: Record<string, number> = {
  jan: 0, feb: 1, mar: 2, apr: 3, may: 4, jun: 5,
  jul: 6, aug: 7, sep: 8, oct: 9, nov: 10, dec: 11,
};

// Parses the provider's stated reset moment out of a quota message. Handles both the bare
// clock form ("try again at 4:30 PM (America/Chicago)") and the dated form the Codex/ChatGPT
// backend actually emits ("try again at Aug 8th, 2026 3:32 AM"). The dated form is anchored
// on the stated calendar day; the bare form keeps the original today-or-tomorrow behaviour,
// which is the only sane reading when no date is given. Returns null when nothing parses --
// callers must NOT substitute a guessed backoff, because retrying an hour into a three-day
// quota outage is what produces an unbounded doomed-retry loop.
function parseProviderQuotaClockReset(error: string, now: Date) {
  ...
  // A month word we do not recognise means we mis-parsed the message; refuse rather than guess.
  const monthWord = match[1] ?? "";
  const explicitMonth = monthWord
    ? PROVIDER_QUOTA_RESET_MONTHS[monthWord.slice(0, 3).toLowerCase()]
    : undefined;
  if (monthWord && explicitMonth === undefined) return null;
  const explicitDay = match[2] ? Number.parseInt(match[2], 10) : null;
  const explicitYear = match[3] ? Number.parseInt(match[3], 10) : null;
  const hasExplicitDate = explicitMonth !== undefined && explicitDay !== null && explicitYear !== null;
  if (hasExplicitDate && (explicitDay! < 1 || explicitDay! > 31)) return null;
  ...
  if (!timeZone) {
    if (hasExplicitDate) {
      return new Date(Date.UTC(explicitYear!, explicitMonth!, explicitDay!, hour, minute));
    }
  ...
  try {
    ...
    const buildRetryAt = (dayOffset: number) => {
      const targetDay = new Date(Date.UTC(
        hasExplicitDate ? explicitYear! : Number(nowParts.year),
        hasExplicitDate ? explicitMonth! : Number(nowParts.month) - 1,
        (hasExplicitDate ? explicitDay! : Number(nowParts.day)) + dayOffset,
        hour,
        minute,
      ));
    ...
    // An explicit calendar date is authoritative -- never roll it forward a day.
    if (hasExplicitDate) return sameDay;
...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  function readProviderQuotaRetryAt(latestRun: LatestIssueRun, now: Date) {
    ...
    // No adapter-supplied timestamp: read the provider's own stated reset moment out of the
    // failure text using the shared parser. Previously this branch fell straight through to a
    // flat +1h, so a reset three days out was retried hourly (~72 doomed runs per agent).
    const stated = parseProviderQuotaClockReset(
      [latestRun?.error ?? "", JSON.stringify(result)].join("\n"),
      now,
    );
    if (stated && stated.getTime() > now.getTime()) return stated;
    // Unknown reset moment. Return null rather than guessing: the caller parks the issue
    // instead of scheduling a retry that cannot succeed.
    return null;
  }
  ...
  async function ensureProviderQuotaWaitRecoveryMonitor(input: {
    ...
    // The provider did not tell us when the quota resets and nothing durable recorded it.
    // Park instead of guessing: the caller has already blocked the issue and attached a
    // board-owned recovery action, so a human decides when to resume. Scheduling a guessed
    // retry here is what turned a multi-day provider outage into an hourly retry loop.
