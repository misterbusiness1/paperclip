...
describe("successful run handoff decision", () => {
  ...
  // Regression: the loop that ran on OXFA-24151 alternated
  // finish_successful_run_handoff -> source_scoped_recovery_action -> finish_successful_run_handoff.
  // The recovery-action arm carries neither handoffRequired nor the handoff wake reason, so it was
  // re-read as fresh productive source work and minted the next handoff. All 536 such runs observed
  // in production carry recoveryActionId, which is what the guard now keys on.
  it("does not loop from a source-scoped recovery-action run", () => {
    expect(decide({
      run: {
        ...run,
        id: "run-3",
