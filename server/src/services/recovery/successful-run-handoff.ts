...
// True when THIS machinery spawned the run. Two arms mint runs for the same
// missing-disposition cause: the corrective handoff wake (handoffRequired /
// FINISH_SUCCESSFUL_RUN_HANDOFF_REASON) and the source-scoped recovery-action wake that
// escalation mints (stamped with recoveryActionId). Both must be excluded as handoff
// *sources* -- otherwise the recovery run is re-read as fresh productive work and mints
// the next handoff, which is the finish_successful_run_handoff <-> source_scoped_recovery_action
// lap. Keyed on the recoveryActionId marker the wake builder already writes, not on a
// wake-reason allowlist, which would drift as new recovery reasons are added.
