...
export function issueRoutes(
  ...
  async function isOwnRecoveryRemediationRun(
    companyId: string,
    ...
    const context = run?.contextSnapshot && typeof run.contextSnapshot === "object"
      ? run.contextSnapshot as Record<string, unknown>
      : null;
    return readNonEmptyString(context?.recoveryActionId) === recoveryActionId;
  }
  ...
  async function revalidateActiveSourceRecovery(input: {
    ...
    // A recovery action must not be invalidated by the state its OWN remediation created.
    // The run this action dispatched carries the action id in its context snapshot; when that
    // run is the actor, the issue's new state IS the remediation working as intended, not
    // evidence the action went stale. Without this guard the recovery run's own
    // blocked -> in_progress update cancels the action, which resets the attempt accounting
    // and re-arms the handoff -> escalate -> recover lap indefinitely.
    // Human/board actors carry no runId, so hand-backs still cancel normally.
    if (await isOwnRecoveryRemediationRun(input.issue.companyId, input.actor, activeRecoveryAction.id)) {
