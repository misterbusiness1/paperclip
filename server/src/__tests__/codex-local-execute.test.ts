...
describe("codex execute", () => {
  ...
  it.each([
    ["command_execution", "env", { type: "command_execution", command: "env" }],
    ["tool_use", "printenv", { type: "tool_use", name: "functions.exec_command", input: { cmd: "printenv" } }],
    ["function_call", "set", { type: "function_call", name: "exec_command", arguments: JSON.stringify({ cmd: "set" }) }],
  ])("blocks broad environment dumps from Codex CLI boundary events: %s", async (_eventKind, commandText, item) => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-codex-guard-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const markerPath = path.join(root, "command-started-marker");
    const logs: LogEntry[] = [];
    await fs.mkdir(workspace, { recursive: true });
    await seedSharedCodexAuth(root);
    const script = `#!/usr/bin/env node
const fs = require("node:fs");
console.log(JSON.stringify({ type: "thread.started", thread_id: "codex-session-guard" }));
console.log(JSON.stringify({ type: "item.started", item: ${JSON.stringify(item)} }));
setTimeout(() => {
  fs.writeFileSync(${JSON.stringify(markerPath)}, "started", "utf8");
  console.log("SHOULD_NOT_BE_LOGGED");
}, 250);
setTimeout(() => {}, 10_000);
`;
    await fs.writeFile(commandPath, script, "utf8");
    ...
    try {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.errorMessage).toContain("runtime command guard rejected");
      expect(result.resultJson?.stdout).toBe("");
      expect(result.resultJson?.stderr).toContain("runtime command guard rejected");
      expect(logs.some((entry) => entry.chunk.includes("SHOULD_NOT_BE_LOGGED"))).toBe(false);
      await expect(fs.stat(markerPath)).rejects.toThrow();
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stderr",
          chunk: expect.stringContaining("runtime_command_guard_broad_dump"),
        }),
      );
    } finally {
      await fs.rm(root, { recursive: true, force: true });
