...
ARG USER_GID=1000
RUN apt-get update \
  && apt-get install -y --no-install-recommends ca-certificates gosu curl gh git wget ripgrep python3 util-linux \
...
RUN echo "cli-tools-epoch: ${CLI_TOOLS_CACHE_EPOCH}" \
  ...
  && apt-get update \
  && apt-get install -y --no-install-recommends openssh-client jq php-cli php-xml composer \
  && rm -rf /var/lib/apt/lists/* \
  && mkdir -p /paperclip \
