...
import {
  evaluateRuntimeCommandGuard,
  inferOpenAiCompatibleBiller,
  type AdapterExecutionContext,
  type AdapterExecutionResult,
  type RuntimeCommandGuardDecision,
...
function parseRuntimeGuardToolArguments(value: unknown): Record<string, unknown> | null {
  if (typeof value === "string") return parseJsonRecord(value);
  return asRecord(value);
}

function isDirectExecToolName(value: unknown): boolean {
  return value === "exec_command" || value === "functions.exec_command";
}

function readRuntimeGuardCommandFromDirectExecTool(item: Record<string, unknown>): string | null {
  if (!isDirectExecToolName(item.name)) return null;
  const input = parseRuntimeGuardToolArguments(item.input ?? item.arguments);
  const command = input?.cmd ?? input?.command;
  return typeof command === "string" && command.trim().length > 0 ? command : null;
}

function readRuntimeGuardCommandFromCodexEvent(line: string): string | null {
  const event = parseJsonRecord(line);
  if (!event || event.type !== "item.started") return null;
  const item = asRecord(event.item);
  if (!item) return null;
  if (item.type === "command_execution") {
    return typeof item.command === "string" && item.command.trim().length > 0 ? item.command : null;
  }
  if (item.type === "tool_use" || item.type === "function_call") {
    return readRuntimeGuardCommandFromDirectExecTool(item);
  }
  return null;
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  try {
    ...
    const runAttempt = async (resumeSessionId: string | null) => {
      ...
      let guardDecision: RuntimeCommandGuardDecision | null = null;
      let guardStdoutBuffer = "";

      const inspectCodexStdout = async (chunk: string) => {
        if (guardDecision && !guardDecision.allowed) return;
        const lines = `${guardStdoutBuffer}${chunk}`.split(/\r?\n/);
        guardStdoutBuffer = lines.pop() ?? "";
        for (const line of lines) {
          const commandText = readRuntimeGuardCommandFromCodexEvent(line);
          if (!commandText) continue;
          const decision = evaluateRuntimeCommandGuard(commandText);
          if (decision.allowed) continue;
          guardDecision = decision;
          if (killTarget) signalCodexChild(killTarget, "SIGTERM");
          await onLog("stderr", `[paperclip] ${decision.message}\n`);
          return;
        }
      };
      ...
      try {
        const proc = await runAdapterExecutionTargetProcess(runId, runtimeExecutionTarget, command, args, {
          ...
          onLog: async (stream, chunk) => {
            ...
            if (stream === "stdout") {
              await inspectCodexStdout(chunk);
              if (guardDecision && !guardDecision.allowed) return;
        ...
        const blockedGuardDecision = guardDecision as RuntimeCommandGuardDecision | null;
        if (blockedGuardDecision && !blockedGuardDecision.allowed) {
          return {
            proc: {
              ...proc,
              exitCode: 1,
              stdout: "",
              stderr: blockedGuardDecision.message,
            },
            rawStderr: blockedGuardDecision.message,
