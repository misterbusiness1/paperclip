...
// Bounds the derived exclude list so a pathological .gitignore can never build
// an unbounded argv for tar.
const MAX_DERIVED_WORKSPACE_EXCLUDES = 256;
...
/**
 * Entries the workspace's own `.gitignore` marks as ignored.
 *
 * Anything git ignores is by definition not workspace content: build output,
 * package caches, scratch directories and vendored checkouts. Shipping them
 * over ssh costs bandwidth and wall-clock on every heartbeat while contributing
 * nothing the next run needs.
 *
 * We ask git for the set rather than parsing `.gitignore` here, so the ignore
 * rules keep exactly one source of truth and there is no second pattern matcher
 * to drift from it. `--directory` collapses a fully-ignored directory to a
 * single entry, which is the shape tar's `--exclude` wants.
 *
 * Returns `[]` for a non-git directory or any git failure. Excluding nothing is
 * the safe direction: it degrades to the previous behaviour rather than risking
 * dropping data.
 */
async function readGitIgnoredEntries(localDir: string): Promise<string[]> {
  try {
    ...
    return [...new Set(entries)].slice(0, MAX_DERIVED_WORKSPACE_EXCLUDES);
  } catch {
    return [];
  }
}
...
/**
 * The local-only path set for a workspace: paths that must not cross the sync
 * boundary in either direction.
 *
 * Callers use this for BOTH `exclude` (what never goes over the wire) and
 * `preserveLocalEntries` (what a restore must not delete locally). Those two
 * must always agree — excluding a path from the remote snapshot without
 * preserving it locally would make the restore merge treat it as deleted.
 * Deriving both from one call is what keeps them from drifting apart.
 */
export async function workspaceLocalOnlyExcludes(localDir: string): Promise<string[]> {
  return [...WORKSPACE_LOCAL_ONLY_EXCLUDES, ...(await readGitIgnoredEntries(localDir))];
}
...
export async function prepareWorkspaceForSshExecution(input: {
  ...
  const localOnly = await workspaceLocalOnlyExcludes(input.localDir);
...
export async function restoreWorkspaceFromSshExecution(input: {
  ...
  if (gitSnapshot) {
    ...
    await syncDirectoryFromSsh({
      ...
      localDir: input.localDir,
      exclude: [".git", ...localOnly],
      preserveLocalEntries: [".git", ...localOnly],
      onProgress: input.onProgress,
      progressLabel: "workspace",
    });
    return;
  ...
  await syncDirectoryFromSsh({
    ...
    localDir: input.localDir,
    exclude: [...localOnly],
    preserveLocalEntries: [...localOnly],
    onProgress: input.onProgress,
    progressLabel: "workspace",
  });
}
