...
import type { AcpPermissionDecision, AcpPermissionRequest, AcpRuntimeOptions } from "acpx/runtime";
...
function commandPermissionRequest(rawInput: unknown, kind: "execute" | "read" = "execute"): AcpPermissionRequest {
  return {
    sessionId: "session-1",
    inferredKind: kind,
    raw: {
      sessionId: "session-1",
      toolCall: { toolCallId: "tool-1", kind, rawInput },
      options: [
...
describe("shared ACPX engine runtime behavior", () => {
  it("installs a pre-execution ACP permission guard for broad environment dumps", async () => {
    const { runtimeOptions } = await runExecutor({ agent: "codex" });
    const hook = runtimeOptions[0]?.onPermissionRequest as
      | ((req: AcpPermissionRequest, ctx: { signal: AbortSignal }) => Promise<AcpPermissionDecision | undefined>)
      | undefined;
    expect(hook).toBeTypeOf("function");

    for (const rawInput of [
      { command: "env" },
      { program: "printenv", args: [] },
      { program: "bash", args: ["-lc", "set"] },
    ]) {
      let markerCreated = false;
      const request = commandPermissionRequest(rawInput);
      const decision = await hook?.(request, { signal: new AbortController().signal });
      if (!decision) markerCreated = true;
      expect(decision, JSON.stringify(rawInput)).toEqual({ outcome: "reject_once" });
      expect(markerCreated).toBe(false);
      expect(evaluateAcpRuntimeCommandPermission(request)).toEqual({ outcome: "reject_once" });
    }

    const quotedSearch = commandPermissionRequest({
      program: "rg",
      args: ["env|printenv|set", "packages/adapters"],
    });
    expect(readRuntimeCommandFromAcpPermissionRequest(quotedSearch)).toContain("'env|printenv|set'");
    expect(await hook?.(quotedSearch, { signal: new AbortController().signal })).toBeUndefined();
    expect(
      await hook?.(commandPermissionRequest({ program: "bash", args: ["-lc", "set -eu; printf ok"] }), {
        signal: new AbortController().signal,
      }),
    ).toBeUndefined();
    expect(evaluateAcpRuntimeCommandPermission(commandPermissionRequest({ command: "env" }, "read"))).toBeUndefined();
  });
