...
export function readRuntimeCommandFromAcpPermissionRequest(req: AcpPermissionRequest): string | null {
  if (req.inferredKind !== "execute" && req.raw.toolCall.kind !== "execute") return null;
  const rawInput = req.raw.toolCall.rawInput;
  if (typeof rawInput === "string") return rawInput.trim() || null;
  if (typeof rawInput !== "object" || rawInput === null || Array.isArray(rawInput)) return null;
  const input = rawInput as Record<string, unknown>;
  const command = input.command ?? input.cmd ?? input.program;
  if (typeof command !== "string" || command.trim().length === 0) return null;
  const rawArgs = Array.isArray(input.args) ? input.args : Array.isArray(input.arguments) ? input.arguments : [];
  const args = rawArgs.filter((value): value is string => typeof value === "string");
  return [command, ...args.map(shellQuote)].join(" ");
}

export function evaluateAcpRuntimeCommandPermission(
  req: AcpPermissionRequest,
): AcpPermissionDecision | undefined {
  const command = readRuntimeCommandFromAcpPermissionRequest(req);
  if (!command || evaluateRuntimeCommandGuard(command).allowed) return undefined;
...
export function createAcpxEngineExecutor(deps: AcpxEngineExecutorOptions = {}) {
  ...
  return async function executeAcpxEngine(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    const runtimeOptions: AcpRuntimeOptions = {
      ...
      onPermissionRequest: async (req) => evaluateAcpRuntimeCommandPermission(req),
