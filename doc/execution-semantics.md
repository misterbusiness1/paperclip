...
- Only a `critical` (P1) assignment may exceed 50. The caller must provide `capacityOverride.reason` with at least 10 non-whitespace characters. A successful override writes activity evidence in the assignment transaction naming the actor, target agent, issue, pre-assignment count, projected count, and reason. Invalid overrides return `422 agent_capacity_override_invalid`.
