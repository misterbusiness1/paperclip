...
Paperclip enforces a per-agent inbound assignment guardrail in the server. An active assignment is an issue whose `assigneeAgentId` is the target agent and whose status is exactly `todo`, `in_progress`, or `in_review`. The indexed count uses `(company_id, assignee_agent_id, status)`.

- At 45 active issues, Paperclip opens one high-priority capacity escalation for the first non-saturated active manager in the reporting chain and emits an `issue_assigned` wake. A partial unique index deduplicates concurrent reconciliation. The episode closes only after the target drops below 45; a later crossing starts a new episode. If every manager is saturated or unavailable, Paperclip records an unroutable escalation instead of adding work to that chain.
- An assignment that moves the target from 49 to 50 is allowed. Further inbound active assignments return `409 agent_at_active_capacity`. Create, reassignment, and checkout perform the advisory lock, indexed count, and assignment write in the same transaction, so concurrent callers cannot exceed the cap.
- The cap applies only when an active issue acquires a new agent owner. Active-to-`blocked`, `backlog`, `done`, or `cancelled` transitions, reassignment away from an agent, release, and lifecycle changes for work the same agent already owns remain available.
- Only a `critical` (P1) assignment may exceed 50. The caller must provide `capacityOverride.reason` with at least 12 non-whitespace characters. A successful override writes activity evidence in the assignment transaction naming the actor, target agent, issue, pre-assignment count, projected count, and reason. Invalid overrides return `422 agent_capacity_override_invalid`.

The experimental `enableActiveAssignmentCapacityGuardrail` setting is the operational kill switch. Enabled mode enforces rejection. Disabled mode is observe-only: over-cap assignments remain visible through `agent.capacity_assignment_observed` activity and are never silently ignored.
